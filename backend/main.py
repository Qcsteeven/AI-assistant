...
async def chat_endpoint(question: dict):
    ...
    try:
        ...
        prompt = f"""
        Ты работаешь как ассистент, который отвечает на вопросы на основе загруженных документов. Ответ должен быть:
        - максимально точным и полным,
        - оформленным в виде нескольких абзацев, если это необходимо,
        - с разделением информации по смысловым блокам.
        Прочитай внимательно предоставленный контекст и ответь на вопрос, используя только информацию из него.
